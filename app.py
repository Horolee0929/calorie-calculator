...
with st.form("nutrition_form"):
    for food in selected_foods:
        unit = "个" if "Boiled Egg" in food else ("块" if "Sandwich Cracks" in food else "g")
        qty = st.number_input(f"{food}（{unit}）", min_value=0.0, step=1.0 if unit in ["个", "块"] else 10.0, key=food)
        gram_qty = qty * 53 if unit == "个" else (qty * 16.25 if unit == "块" else qty)
